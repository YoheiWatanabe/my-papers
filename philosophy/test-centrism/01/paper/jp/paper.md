...
TDDではまず テストを書くことから始める。最初のテスト $X_1$ を
...
さらに`gcd8`は単純な割り算探索(brute-force)の例である。
...
[TDD開発の例](#24-tdd開発の例)で示した最大公約数のプログラム`gcd1`〜`gcd8`について考えてみよう。
...
主体が対象 $T \in \mathcal T$ を**理解する**とは任意の対象 $S \in \mathcal T$ について
